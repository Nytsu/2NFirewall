...
public class ActivityMain extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    //Neural Network Driver File Variables
    public static double [][] input = new double[1027][5]; // inputs the five main package attributes
    public static double [][] Y = new double[1027][1]; // outputs a 1.0 if the package is ACCEPTED or 0.0 if DENIED
    public static int weight = 13; // Initial value of neuron weights
    public static NeuralNetwork nn = new NeuralNetwork(5,weight,1); // create a neural network with 5 input and 1 output
    public static Random rand = new Random(); // used to generate a random weight. Weights are initialized with a random number
    public static int k = 0; // counts the number of corretly predicted outputs
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ivQueue.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            /*new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    Intent homeIntent = new Intent(ActivityMain.this, NNAct.class)
                    startActivity(NNAct);
                    finish();
            ...
            button2n = findViewById(R.id.Button2N);
            swEnabled.setChecked(enabled);

            // ********************************* 2NFIREWALL SECTION ************************************
            //  2NFirewall Button Functionality

            button2n.setOnClickListener(new View.OnClickListener(){
                @Override
                public void onClick(View view) {
                    ...
                    if (i % 2 != 0) {
                        //count = count - 1;
                        Log.i("2NFirewall", "Enable 2NFirewall");
                        swEnabled.setChecked(true);

                    } else if (i % 2 == 0) {
                        //count = count + 1;
                        Log.i("2NFirewall", "disable 2NFirewall");
                        swEnabled.setChecked(false);
                ...
                if (isChecked) {
                    ...
                    try {
                        ...
                        if (prepare == null) {
                            ...
                            View view = inflater.inflate(R.layout.vpn, null, true);
                    ...
                    /*Toast on = Toast.makeText(ActivityMain.this, R.string.msg_2n_off, Toast.LENGTH_LONG);
    ...
    */
        ...
        if (!initialized) {
            ...
            readFromCSV(); // read from csv method call to read contents of packet file data in csv format and assign the values to input[][] and Y[][]
            testNeuralNetwork(); // test neural network method call to test prediction capabilities of neural network
    ...
    protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == REQUEST_VPN) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                Toast on = Toast.makeText(ActivityMain.this, R.string.msg_2n_on, Toast.LENGTH_LONG);
                on.setGravity(Gravity.CENTER, 0, 0);
                on.show();
            ...
            if (adapter != null)
                if (intent.hasExtra(EXTRA_CONNECTED) && intent.hasExtra(EXTRA_METERED)) {
                    ivIcon.setImageResource(Util.isNetworkActive(ActivityMain.this)
                            ? R.drawable.ic_nnfirewall_noti_white
                            : R.drawable.ic_nnfirewall_noti);
...
//                return true;
    ...
    //Below this Neural Network training code starts

    public void readFromCSV() { // The following method reads the data from the csv file

        /* The following code reads the values from the data.csv file
         * and inserts values into input[][] and Y[][]
         * */


        ClassLoader classLoader = ActivityMain.class.getClassLoader(); // location of csv file with packet information
        InputStream file = getResources().openRawResource(R.raw.trainingdata); // Create a file object with the filename path
        //InputStream file = classLoader.getResourceAsStream("trainingdata.csv"); // Create a file object with the filename path
        List<List<String>> lines = new ArrayList<>();// this gives you a 2-dimensional array of strings

        Scanner inputStream; // Create an inputStream object to read file contents

        inputStream = new Scanner(file);
        inputStream.skip("uid,IPversion,protocol,dIPaddr,dport,allowed,"); // skip first line of data.csv file

        while(inputStream.hasNext()){
            String line= inputStream.next();
            String[] values = line.split(",");
            lines.add(Arrays.asList(values)); // this adds the currently parsed line to the 2-dimensional string array
        }
        ...
        for(List<String> line: lines) {
            ...
            for (String value: line) {
                ...
                /* Input all colummns from the csv file except 4 and 6 into the input[][] array
                 * */
                if(columnNo != 4 && columnNo != 6) {
                    input[lineNo - 1][columnNo - 1] = Double.parseDouble(value);
                    //System.out.println( "The packet info is: " +input[lineNo - 1][columnNo - 1]);

                }


                /* Convert Ip address into double value so that the nn can process
                 * */
                else if(columnNo == 4) {
                    String ip= value; // Stores the ip address value in a string
                    //System.out.println("This is the ip: " + value); //Displays the ip




                    double num = 0; // Stores the ip address value in number form
                    num =  ip.hashCode() % 64;
                    //System.out.println(num);
                    input[lineNo - 1][columnNo - 1] = num;
                ...
                /* 6th column of the csv file is inserted into Y[][] array this is the netguard output array
                 * */
                else if(columnNo == 6) {

                    if(value.contains("false")) // If the packet is not allowed
                    {
                        Y[lineNo-1][0] = 0.0; // Assign a value of 0.0 to output array
                        //System.out.println("Is the packet accepted: "+Y[lineNo - 1][0]); //
                    }

                    else if(value.contains("true")){ // If the packet is allowed
                        Y[lineNo-1][0] = 1.0; // Assign a value of 1.0 to output array
                        //System.out.println("Is the packet accepted: "+Y[lineNo - 1][0]);
                    }
                    else { // Store column 6 output column in the Y(output) array
                        Y[lineNo-1][0] = Double.parseDouble(value);
                    }

                }


                columnNo++; // Column number iterator
            }
            lineNo++; // Column number iterator
        }
    ...
    public static void trainNeuralNetwork() {
        ...
        nn = new NeuralNetwork(5,weight,1);
        nn.fit(input, Y, 100000); // train the neural network with packet data input and output
    ...
    public static void testNeuralNetwork() {
        ...
        /*The following code predicts the output using the neural network
         *
         * */


        boolean isTrained = false; // when true means that the neural network has been trained
        double outputdecimal = 0.0; // stores the predicted output value of the nn
        int j = 0; // iteration variable for Y[][]


        /* While loop until the neural network has been trained
         * The weights are changed in each iteration
         * The change of weights is what creates the learning process
         *
         * */
        long start = System.currentTimeMillis(); // Start timer
        while(isTrained != true) {


            trainNeuralNetwork();


            List<Double>output; // create a list of outputs
            j = 0;
            k = 0;
            for(double d[]:input)  // Test the neural network to see if it is outputting the correct result
            {


                output = nn.predict(d); // neural network predicts the output and stores it inside the List<Double>output;
                outputdecimal = output.get(0); // desired output is stored inside the outputdecimal variable

                /* If nn output is more than 0.5 value is true (1.0) see sigmoid function for explanation
                 * */
                if(outputdecimal > 0.5) {
                    outputdecimal = 1.0;
                }

                /* If nn output is less than 0.5 value is false (0.0)
                 * */
                else if(outputdecimal < 0.5) {
                    outputdecimal = 0.0;
                }

                /* The output of the nn and the output of the netguard program
                 * */
                //System.out.println("The output of the nn is: " + outputdecimal + " The real output should be: " + Y[j][0]);


                /*Count each time the predicted output of the neural network matches the output of Netguard firewall (packet.allowed)
                 * */
                if(outputdecimal == Y[j][0]) {
                    k = k + 1;
                }

                /* Stop loop when k neural network outputs match k netguard outputs
                 * */
                if(k >= (993)) {
                    isTrained = true;
                }
                ...
                /* Iteration variable for Y[][] which is the variable that contains the netguard output
                 * */
                j = j + 1;
            }
            /* The weight of the neural network is displayed for reference
             * */
        ...
        long timeEllapsed = (end - start) / 1000; // calculate the number of seconds ellapsed
        System.out.println("The neural network has been trained!!"); // The neural network has been trained
        System.out.println("The elapsed time for training has been: "+ timeEllapsed + " seconds" ) ; // Output the ellapsed time

    }

    //Neural Network Training Code ends here
