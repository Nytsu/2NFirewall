...
public class ServiceSinkhole extends VpnService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private Allowed isAddressAllowed(Packet packet) {
        ...
        String ip= packet.daddr; // Stores the ip address value in a string
        double outputdecimal = 0.0; // stores the predicted output value of the nn
        double ipaddress = 0; // Stores the ip address value in number form
        ipaddress =  ip.hashCode() % 64;

        double[] packetData = {(double)packet.uid, (double)packet.version, (double)packet.protocol, ipaddress, (double)packet.dport};

        NeuralNetwork nn = ActivityMain.nn;

        output = nn.predict(packetData); // neural network predicts the output and stores it inside the List<Double>output;
...
/**
        try {
            ...
            if (myObj.createNewFile()) {
                ...
                myWriter.write("uid,IPversion,protocol,dIP,dport,ACCEPT,\r\n");
                String writeToFile = packet.uid + "," + packet.version +"," + packet.protocol + "," + packet.daddr + "," + packet.allowed + ",\r\n";
        ...
        File myFile = new File(getExternalCacheDir(),  "test.csv");

        Log.i("File location: ","This is the path" + myFile);
        //NEW CODE ADDED TO GENERATE AND ADD DATA TO CSV FILE
        ...
        try
        {
            ...
            {
                Log.i(TAG1,"File created: " + myFile.getName());
                myFile.mkdir();
                FileWriter myWriter = new FileWriter(myFile, true);
                myWriter.write("uid" + "," + "IPversion" + "," + "protocol" + "," + "dIPaddr" + "," + "dport" + "," + "allowed" + ",\r\n");
        ...
        try
        {
            ...
            {
                FileWriter myWriter = new FileWriter(myFile, true);
                myWriter.write(packet.uid + "," + packet.version + "," + packet.protocol + "," + packet.daddr + "," + packet.dport + "," + packet.allowed + ",\r\n");
                myWriter.close();
                Log.i(TAG1,"Successfully wrote to the file.");
            }
        } catch (IOException e)
        {
            Log.i(TAG1,"An error occurred2.");
            e.printStackTrace();
        }
...
**/
        ...
        if (prefs.getBoolean("log", false) || prefs.getBoolean("log_app", false))
            if (packet.protocol != 6 /* TCP */ || !"".equals(packet.flags))
                if (packet.uid != Process.myUid())
                    logPacket(packet);
        String isallowed = String.valueOf(packet.allowed);
        Log.i("Packet","This are the values of the packet  " + packet.toString() + " is it allowed =  " + isallowed );

        Log.i("packet log", packet.uid + "," + packet.version + "," + packet.protocol + "," + packet.daddr + "," + packet.dport + "," + packet.allowed + ",");
    ...
    public void notifyNewApplication(int uid) {
        ...
        try {
            ...
            builder.setSmallIcon(R.drawable.ic_nnfirewall_noti_white)
                    .setContentIntent(pi)
                    .setColor(tv.data)
                    .setAutoCancel(true);
    ...
    private Notification getEnforcingNotification(int allowed, int blocked, int hosts) {
        ...
        builder.setSmallIcon(isLockedDown(last_metered) ? R.drawable.ic_lock_outline_white_24dp : R.drawable.ic_nnfirewall_noti_white)
                .setContentIntent(pi)
                .setColor(tv.data)
                .setOngoing(true)
                .setAutoCancel(false);
